...
class ProductController extends Controller
{
    ...
    public function upload(Request $request)
    {
        ...
        Log::debug(request()->all());
        $user = Auth::user();
        $user->notify(new \App\Notifications\UploadProductNotification());
    ...
    public function getTopSellingProducts(Request $request)
    {
        ...
        $startDate = $startDate ? Carbon::parse($startDate) : Carbon::now()->subDays(7);
        ...
        switch ($timeFilter) {
            case 'week':
                // query already filters for the past week
                break;
            case 'month':
        ...
        return response()->json(['products' => $topSellingProducts]);
    ...
    public function cancelOrder(Request $request)
    {
        try {
            Log::debug($request->all());
            $product = Products::where('order_id', $request->order_id)->first();
            if (!$product) {
                $product->transaction_type = 'refund';
                $product->save();
            }
